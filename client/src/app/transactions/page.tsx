...
export default function TransactionsPage() {
  ...
  async function load(p: number) {
    setLoading(true);
    try { const d = await api.transactions(p); setTxns(d.transactions); setTotal(d.total); }
    ...
    <div style={s.page}>
      ...
      <main style={s.main}>
        ...
        <div style={s.tableWrap}>
          <table style={s.table}>
            <thead><tr>
              {["Date","User Wallet","Chain","Amount","Plan","Tx Hash","Status"].map(h => <th key={h} style={s.th}>{h}</th>)}
            </tr></thead>
            <tbody>
              {loading
                ...
                : txns.map(t => (
                  <tr key={t.id} style={s.tr}>
                    ...
                    <td style={s.td}><span style={s.chain}>{t.chain}</span></td>
        ...
        <div style={s.pagination}>
          ...
          <span style={s.pageInfo}>Page {page} of {pages} · {total} total</span>
...
function shorten(s: string|null) { return !s ? "—" : s.length>16 ? `${s.slice(0,8)}…${s.slice(-6)}` : s; }
...
const s: Record<string, React.CSSProperties> = {
  ...
  chain:      { display:"inline-block", padding:"2px 8px", borderRadius:4, fontSize:11, fontWeight:700, background:"#1e3a5f", color:"#93c5fd" },
